...
void setup() {
  ...
  cactussen = new Cactus[2];
  cactussen[0] = new Cactus(width); //dit vult de array met de cactus
  cactussen[1] = new Cactus(width-100);
}
...
void draw() {
  ...
  if (!gameOver) {
    ...
    for (int i = 0; i < cactussen.length; i++) {
      Cactus cactus = cactussen[i];
      cactus.update();
      cactus.draw();
...
void resetGame() {
  ...
  score = 0;
  for (int i = 0; i < cactussen.length; i++) {
    cactussen[i].x = width + i * 100;
    cactussen[i].speed = 7;
  }
}

//Code waar ik trost op ben
